...
/**
 * Internal type for an event handler that accepts any event payload.
 */
type AnyQueueHandler = (payload: QueueEventMap[QueueEvent]) => void;

/**
 * Holds the currently registered listeners for each queue event type.
 * Listeners are managed per event, with no duplicates.
 */
const listeners: Record<QueueEvent, Set<AnyQueueHandler>> = {
  "queue-processed": new Set(),
  "request-success": new Set(),
  "request-failed": new Set(),
  "request-processed": new Set(),
  "connection-lost": new Set(),
  "connection-restored": new Set(),
};

/**
 * Generic event handler type for a specific offline queue event.
 */
export type QueueEventHandler<E extends QueueEvent> = (
  payload: QueueEventMap[E],
) => void;
 ...
 */
export const onQueueEvent = <E extends QueueEvent>(
  event: E,
  handler: QueueEventHandler<E>,
): (() => void) => {
  listeners[event].add(handler as AnyQueueHandler);

  // Returns an unsubscribe function for convenience.
  return () => {
    listeners[event].delete(handler as AnyQueueHandler);
  };
};
 ...
 */
export const offQueueEvent = <E extends QueueEvent>(
  event: E,
  handler: QueueEventHandler<E>,
): void => {
  listeners[event].delete(handler as AnyQueueHandler);
 ...
 */
export const emitQueueEvent = <E extends QueueEvent>(
  event: E,
  payload: QueueEventMap[E],
): void => {
  const eventListeners = listeners[event];

  // Iterate and invoke each registered handler for this event.
  eventListeners.forEach((handler) => {
    try {
      handler(payload);
    } catch (error) {
      // Prevent any single listener from breaking propagation.
      // Log clear, actionable error to the developer.
      // eslint-disable-next-line no-console
      console.error(
        `[Offline Queue] Listener for event "${event}" threw an error. This does not affect other listeners.\nDetails:`,
        error,
      );
    }
  });
